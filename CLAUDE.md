...
Requires a JDK 19 (Gradle toolchain, auto-detected from `~/.jdks/` etc.).

Test reports land in `build/reports/tests/test/index.html`.

## External dependency: probability-math

The probability engine (`math` package: `Probability`, `MathFunctions`,
`StochasticFunctions`, `Polynom`) lives in the separate `probability-math` repo
and is consumed as `com.stats:probability-math`. It is wired in via Gradle
**composite build** — `includeBuild '../probability-math'` in `settings.gradle` —
so `probability-math` must be checked out as a sibling folder. Without the
sibling, run `gradlew.bat publishToMavenLocal` in the library repo and comment
out the `includeBuild` line to fall back to `~/.m2`.
